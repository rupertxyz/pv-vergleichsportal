...
if (dark) {
  // Flussgrafik an Theme anpassen
  $('.flussgrafik').attr(
    'src',
    'images/unabhaengigkeitsrechner-energiefluesse-dark.svg'
  );
}
// Switch light/dark mode recognition
window
  .matchMedia('(prefers-color-scheme: dark)')
  .addEventListener('change', (e) => e.matches && pageReload());
window
...
//Setup fÃ¼r Ajax Requests (getJSON, getScript etc.)
$.ajaxSetup({
  cache: true,
...
//Initialisierung und Event-Handler fÃ¼r Last-Slider
$(function () {
...
//Initialisierung und Event-Handler fÃ¼r PV-Slider
$(function () {
...
//Initialisierung und Event-Handler fÃ¼r Batterie-Slider
$(function () {
...
//Event-Handler fÃ¼r Text-Input Stromverbrauch
$(function () {
  $('#amount-last').on('change', function () {
    ...
    if (
      !IsNumber(invariantValue) ||
      invariantValue < min ||
      invariantValue > max
    ) {
      alert(
        'Bitte geben Sie einen gÃ¼ltigen Stromverbrauch im Bereich von ' +
          min +
          ' bis ' +
          max +
          ' kWh ein.'
    ...
    SetRatioPV(
      $('#slider-range-last').slider('value'),
      $('#slider-range-pv').slider('value') / 10
    );
    SetRatioBat(
      $('#slider-range-last').slider('value'),
      $('#slider-range-bat').slider('value')
    );
    GetResults();
  });
});

//Event Handler fÃ¼r Text-Input PV
$(function () {
  $('#amount-pv').on('change', function () {
    ...
    if (
      ...
      alert(
        'Bitte geben Sie eine gÃ¼ltige PV-Leistung im Bereich von ' +
          min / 10 +
          ' bis ' +
          max / 10 +
          ' kW ein.'
...
//Event Handler fÃ¼r Text-Input Batterie
$(function () {
  $('#amount-bat').on('change', function () {
    ...
    if (
      ...
      alert(
        'Bitte geben Sie eine gÃ¼ltige BatteriekapazitÃ¤t im Bereich von ' +
          min +
          ' bis ' +
          max +
          ' kWh ein.'
...
//Hilfsfunktion zum Berechnen des VerhÃ¤ltnisses PV/Last
function SetRatioPV(last, pv) {
  ratio_pv = GetProzentLabels((pv / last) * 1000);
}

//Hilfsfunktion zum Berechnen des VerhÃ¤ltnisses Bat/Last
...
//Funktion zum Ermitteln aller ErgebnisgrÃ¶ÃŸen
...
//Hilfsfunktion zum Runden auf 2 Nachkommastellen fÃ¼r die Charts
      ...
      datasets: [
        {
          ...
          data: [
            GetProzentCharts(dir_eigen),
            GetProzentCharts(eigen - dir_eigen),
            GetProzentCharts(1 - eigen),
          ],
          backgroundColor: ['#FFC000', '#76B500', '#BFBFBF'],
          borderColor: [borderCol, borderCol, borderCol],
          borderWidth: 4,
        },
      ],
    },
          ...
          callbacks: {
            label: function (context) {
              return (
                ...
                ': ' +
                FormatNumber(
                  Math.round(10 * context.dataset.data[context.dataIndex]) / 10
                ) +
                ' %'
              );
            },
      ...
      datasets: [
        {
          ...
          data: [
            GetProzentCharts(dir_autarkie),
            GetProzentCharts(autark - dir_autarkie),
            GetProzentCharts(1 - autark),
          ],
          backgroundColor: ['#FFC000', '#336600', '#595959'],
          borderColor: [borderCol, borderCol, borderCol],
          borderWidth: 4,
        },
      ],
...
var numberFormatterWithMin1Digit = new Intl.NumberFormat(cfg['culture'], {
  useGrouping: false,
  minimumFractionDigits: 1,
});
var numberFormatterWithoutDigit = new Intl.NumberFormat(cfg['culture'], {
  useGrouping: false,
...
function FormatNumber(number) {
  return number > 100
    ? numberFormatterWithoutDigit.format(number)
    : numberFormatterWithMin1Digit.format(number);
}
...
function SetOffsetCenterLabels(valCenterLabel, idCenterLabel) {
  ...
  document
    .getElementById(idCenterLabel)
    .style.setProperty('left', 'calc(50% - ' + offset + 'px)');
}
...
function CopyLinkToClipboard() {
  ...
  var link =
    'https://solar.htw-berlin.de/rechner/unabhaengigkeitsrechner/?l=' +
    last +
    '&p=' +
    pv +
    '&b=' +
